...
@section('content')
<div class="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans selection:bg-blue-500/30">
    <style>
        :root {
                    ...
                    <div class="flex flex-col min-w-0">
                        <nav aria-label="Breadcrumb" class="hidden xs:block">
                            <ol class="flex items-center space-x-2 label-caps text-[9px] sm:text-[10px]">
                                ...
                                <li class="text-slate-500 opacity-60 italic truncate">Création</li>
                            </ol>
                        </nav>
                        <h1 class="text-base sm:text-lg font-black text-slate-100 tracking-tight leading-none mt-1 uppercase truncate">Nouveau Contact</h1>
                    </div>
                </div>
                
                <!-- Right: Command Actions -->
                <div class="flex items-center justify-start sm:justify-end gap-2 sm:gap-3">
                    <a href="{{ route('contacts.index') }}" class="px-4 sm:px-5 py-2 sm:py-2.5 text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-100 hover:bg-white/5 border border-white/5 rounded-xl transition-all">
                        Abandonner
                    </a>
                    <button type="submit" form="create-contact-form" class="px-5 sm:px-7 py-2 sm:py-2.5 text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-900/20 transition-all active:scale-95">
                        <span class="sm:hidden">Enregistrer</span>
                        <span class="hidden sm:inline">ENREGISTRER LA FICHE</span>
                    </button>
                </div>
            </div>
        </div>
