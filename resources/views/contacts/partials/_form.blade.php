            ...
            <div class="p-6">
                ...
                <div class="mb-8 flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-slate-100">
                    <div class="relative group" x-data="{ photoPreview: '{{ $isEdit ? $contact->avatar_url : 'https://ui-avatars.com/api/?name=N+A&background=6366f1&color=fff&size=256' }}' }">
                        ...
                        <label for="photo" class="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 rounded-full cursor-pointer transition-opacity">
                    ...
                    <div class="flex-1 text-center sm:text-left">
                        <h4 class="text-sm font-bold text-slate-900 mb-1">Photo du Contact</h4>
                        <p class="text-xs text-slate-500 mb-3">Format recommandé : Carré, max 2Mo (JPG, PNG, WebP)</p>
                        <button type="button" @click="document.getElementById('photo').click()" class="inline-flex items-center px-3 py-1.5 bg-white border border-slate-300 rounded-md text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
                            Changer la photo
                        </button>
                    </div>
                </div>
