    ...
    <div class="lg:col-span-8 space-y-6">
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
                ...
                <h2 class="text-xs font-bold text-slate-700 uppercase tracking-widest">Identité et Structure</h2>
            </div>
            <div class="p-8">
                <div class="flex flex-col md:flex-row gap-10">
                    ...
                    <div class="shrink-0 flex flex-col items-center gap-4">
                        <div class="relative group" x-data="{ photoPreview: '{{ $isEdit ? $contact->avatar_url : '' }}' }">
                            <div class="h-44 w-44 rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-colors group-hover:border-indigo-300">
                            ...
                            <button type="button" @click="$refs.photoInput.click()" 
                                class="absolute -bottom-3 inset-x-4 h-10 bg-white border border-slate-200 rounded-lg shadow-lg flex items-center justify-center gap-2 text-[10px] font-bold text-slate-700 hover:bg-indigo-700 hover:text-white transition-all active:scale-95 z-30">
                    ...
                    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6">
                        <div class="col-span-1 space-y-1.5">
                            ...
                            <input type="text" name="prenom" id="prenom" value="{{ old('prenom', $isEdit ? $contact->prenom : '') }}" required 
                                class="block w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-900 bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 transition-all outline-none">
                        </div>
                        ...
                        <div class="col-span-2 space-y-1.5 pt-2">
                            <label for="entreprise" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Organisation / Entreprise</label>
                            <div class="relative group">
                                ...
                                <input type="text" name="entreprise" id="entreprise" value="{{ old('entreprise', $isEdit ? $contact->entreprise : '') }}" required
                                    class="block w-full px-4 py-3 pl-10 border border-slate-300 rounded-lg text-sm text-slate-900 bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 transition-all outline-none">
                            </div>
                        </div>

                        <div class="col-span-2 space-y-1.5 border-t border-slate-100 pt-6 mt-2">
                            <label for="poste" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Intitulé de Poste / Fonction</label>
                            <input type="text" name="poste" id="poste" value="{{ old('poste', $isEdit ? $contact->poste : '') }}" 
                                class="block w-full px-4 py-3 border border-slate-300 rounded-lg text-sm text-slate-900 bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-600/5 transition-all outline-none">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
                <div class="h-4 w-1 bg-indigo-600 rounded-full"></div>
                <h2 class="text-xs font-bold text-slate-700 uppercase tracking-widest">Canaux de Communication</h2>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-8">
                    <div class="space-y-1.5">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $isEdit ? $contact->email : '') }}" required 
                            class="block w-full px-4 py-3 border border-slate-300 rounded-lg text-sm bg-slate-50/20 focus:bg-white focus:border-indigo-600 transition-all outline-none">
                    </div>
                    <div class="space-y-1.5">
                        <label for="telephone" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ligne Directe / GSM</label>
                        <div class="iti-container">
                            <input type="tel" name="telephone_input" id="telephone" value="{{ old('telephone', $isEdit ? $contact->telephone : '') }}" 
                                class="block w-full px-4 py-3 border border-slate-300 rounded-lg text-sm focus:border-indigo-600 focus:ring-0 outline-none">
                            <input type="hidden" name="telephone" id="telephone_full" value="{{ old('telephone', $isEdit ? $contact->telephone : '') }}">
                        </div>
                    </div>
                </div>

                <div class="space-y-1.5 border-t border-slate-100 pt-8">
                    <label for="adresse" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Adresse Postale / Siège</label>
                    <div class="relative group">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-300 group-focus-within:text-indigo-600 transition-colors">
                        ...
                        <input type="text" name="adresse" id="adresse" value="{{ old('adresse', $isEdit ? $contact->adresse : '') }}" 
                            class="block w-full px-4 py-3 pl-12 border border-slate-300 rounded-lg text-sm bg-white focus:border-indigo-600 outline-none transition-all">
                    </div>
                </div>

                <!-- Secondary Data (Enterprise Collapsible) -->
                <div x-data="{ open: false }" class="border-t border-slate-100 pt-8">
                    ...
                    <div x-show="open" x-collapse x-cloak class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-10">
                        <div x-data="{ emails: {{ json_encode(old('alternative_emails', ($isEdit && $contact->alternative_emails) ? $contact->alternative_emails : [])) }} }" class="space-y-4">
                            <h4 class="text-[9px] font-bold text-slate-300 uppercase tracking-widest border-b border-slate-100 pb-1">Emails Alias</h4>
                            <div class="space-y-2">
                                <template x-for="(email, index) in emails" :key="index">
                                    <div class="flex gap-1 group">
                                        <input type="email" :name="'alternative_emails['+index+']'" x-model="emails[index]" class="block w-full h-10 px-3 border border-slate-200 rounded-lg text-xs bg-slate-50/50 focus:bg-white focus:border-indigo-600 outline-none transition-all">
                        ...
                        <div x-data="{ phones: {{ json_encode(old('alternative_telephones', ($isEdit && $contact->alternative_telephones) ? $contact->alternative_telephones : [])) }} }" class="space-y-4">
                            <h4 class="text-[9px] font-bold text-slate-300 uppercase tracking-widest border-b border-slate-100 pb-1">Lignes Seconds</h4>
                            <div class="space-y-2">
                                <template x-for="(phone, index) in phones" :key="index">
                                    <div class="flex gap-1 group">
                                        <input type="tel" :name="'alternative_telephones['+index+']'" x-model="phones[index]" class="block w-full h-10 px-3 border border-slate-200 rounded-lg text-xs bg-slate-50/50 focus:bg-white focus:border-indigo-600 outline-none transition-all">
    ...
    <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-[100px]">
        ...
        <div class="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
            ...
            <div class="p-6 space-y-8">
                ...
                <!-- Administrative Field -->
                <div class="space-y-2">
                    <label for="source" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Origine / Acquisition</label>
                    <div class="relative">
                        <select id="source" name="source" 
                            class="block w-full h-12 px-4 border border-slate-300 rounded-lg text-xs bg-white focus:border-indigo-600 outline-none cursor-pointer appearance-none">
                ...
                <div class="space-y-2 pt-2">
                    <label for="tags" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Segmentation Tags</label>
                    <input type="text" name="tags_input" id="tags" value="{{ old('tags_input', ($isEdit && $contact->tags) ? implode(', ', $contact->tags) : '') }}" 
                        class="block w-full h-12 px-4 border border-slate-300 rounded-lg text-xs bg-white focus:border-indigo-600 outline-none transition-all">
                    <p class="text-[8px] font-medium text-slate-400 uppercase italic mt-2 tracking-tight">Séparez les mots-clés par des virgules</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#0f172a] border border-indigo-900/30 shadow-2xl rounded-xl overflow-hidden relative">
            ...
            <div class="p-0">
                ...
                <div class="px-6 py-3 bg-indigo-900/40 text-[9px] font-black text-indigo-400 uppercase flex items-center gap-2 border-t border-indigo-900/20">
...
<style>
    ...
    .iti__selected-dial-code { font-weight: 700 !important; color: #0f172a !important; font-size: 0.81rem !important; }
    .iti__country-list { 
