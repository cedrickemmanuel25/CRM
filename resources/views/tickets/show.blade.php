                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            ...
                            <span class="text-xs text-slate-400 font-medium tracking-tight">Ouvert par {{ $ticket->creator->name }} • {{ $ticket->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
                            ...
                            <li>
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex items-start space-x-4">
                                        <div class="relative">
                                            <div class="h-10 w-10 flex items-center justify-center rounded-xl shadow-md border {{ $activity->user->isSupport() ? 'bg-indigo-600 text-white border-indigo-700' : 'bg-white text-slate-500 border-slate-200' }} font-black text-sm transition-transform hover:scale-110">
                                                {{ substr($activity->user->name, 0, 1) }}
                                            </div>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="bg-white rounded-2xl shadow-sm border {{ $activity->user->isSupport() ? 'border-indigo-100 bg-indigo-50/10' : 'border-slate-200' }} p-6 transition-all hover:shadow-md">
                                                <div class="flex items-center justify-between mb-3">
                                                    <div class="flex items-center gap-2">
                                                        <p class="text-sm font-black text-slate-900">{{ $activity->user->name }}</p>
                                                        @if($activity->user->isSupport())
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden group">
                    ...
                    <div class="p-5 space-y-4">
                        <div class="flex flex-col">
                            <p class="text-base font-black text-slate-900 tracking-tight leading-tight">{{ $ticket->contact->nom_complet }}</p>
                            <p class="text-[10px] font-black text-indigo-600 uppercase tracking-widest mt-0.5">{{ $ticket->contact->entreprise ?? 'Client Indépendant' }}</p>
                        </div>
                        ...
                        <div class="space-y-2 pt-2 border-t border-slate-100">
                            <div class="flex items-center text-xs font-semibold text-slate-600">
                                ...
                                <span class="truncate">{{ $ticket->contact->email }}</span>
                            </div>
                            @if($ticket->contact->telephone)
                            <div class="flex items-center text-xs font-semibold text-slate-600">
                                <div class="h-6 w-6 rounded bg-slate-50 flex items-center justify-center mr-3 text-slate-400 group-hover:text-indigo-500 transition-colors">
