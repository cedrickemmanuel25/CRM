                            ...
                            @if(isset($pipeline[$key]))
                                ...
                                <div class="kanban-card bg-white border-2 border-gray-200 rounded-lg p-3 hover:border-{{ $config['color'] }}-400 hover:shadow-md transition-all sm:cursor-grab sm:active:cursor-grabbing group" data-id="{{ $opp->id }}">
                                    ...
                                    <div class="flex items-center justify-between mb-2">
                                        ...
                                        <form action="{{ route('opportunities.destroy', [$opp, 'view' => request('view')]) }}" method="POST" onsubmit="return confirm('Supprimer cette opportunité ?');" class="inline opacity-0 group-hover:opacity-100 transition-opacity">
                                            @csrf @method('DELETE')
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(auth()->user()->hasRole(['admin', 'commercial']) && (auth()->user()->isAdmin() || $opp->commercial_id === auth()->id()))
                            <form action="{{ route('opportunities.destroy', [$opp, 'view' => request('view')]) }}" method="POST" onsubmit="return confirm('Supprimer cette opportunité ?');" class="inline">
                                @csrf @method('DELETE')
