    <div class="relative max-w-[1800px] mx-auto px-4 sm:px-4 lg:px-4 pb-8">
        ...
        <div x-show="viewMode === 'pipeline'" class="pb-6">
            <div class="flex gap-1.5 w-full items-start overflow-x-auto pb-4">
                @php
                    $stages = [
        ...
        <!-- VUE LISTE -->
        <div x-show="viewMode === 'list'" x-transition class="bg-white rounded-lg border border-gray-200 overflow-hidden flex flex-col">
            <div class="overflow-x-auto">
                <table class="w-full table-fixed divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[20%]">Opportunité</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[15%]">Contact</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[10%]">Montant</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[10%]">Probabilité</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[10%]">Stade</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[10%]">Échéance</th>
                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider w-[15%]">Commercial</th>
                            @if(!auth()->user()->isSupport())
                            <th class="px-3 py-3 text-right w-[10%]"><span class="sr-only">Actions</span></th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors group">
                            <td class="px-3 py-4 break-words">
                                <a href="{{ route('opportunities.show', $opp) }}" class="text-sm font-medium text-gray-900 hover:text-indigo-600 block truncate" title="{{ $opp->titre }}">{{ $opp->titre }}</a>
                                <p class="text-xs text-gray-500 mt-1">{{ $opp->updated_at->diffForHumans() }}</p>
                            </td>
                            <td class="px-3 py-4 break-words">
                                <p class="text-xs font-medium text-gray-900 truncate" title="{{ $opp->contact?->entreprise ?? 'Particulier' }}">{{ $opp->contact?->entreprise ?? 'Particulier' }}</p>
                                <p class="text-[10px] text-gray-500 truncate">{{ $opp->contact?->prenom }} {{ $opp->contact?->nom }}</p>
                            </td>
                            <td class="px-3 py-4">
                                <p class="text-sm font-bold text-gray-900">{{ format_currency($opp->montant_estime) }}</p>
                            ...
                            <td class="px-3 py-4">
                                ...
                                @endphp
                                <span class="inline-flex px-2 py-1 rounded-full text-[10px] font-bold uppercase bg-{{ $color }}-100 text-{{ $color }}-700 truncate max-w-full">{{ $opp->stade }}</span>
                            </td>
                            <td class="px-3 py-4">
                                @if($opp->date_cloture_prev)
                            ...
                            <td class="px-3 py-4">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs font-medium text-gray-700 truncate" title="{{ $opp->commercial?->name ?? 'Non assigné' }}">{{ $opp->commercial?->name ?? 'Non assigné' }}</span>
                                </div>
                            </td>
                            @if(!auth()->user()->isSupport())
