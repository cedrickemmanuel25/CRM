...
@foreach($mainLinks as $link)
    ...
    <li>
        <a href="{{ route($link['route']) }}" 
           class="group flex items-center rounded-xl py-2 px-3 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 
           {{ request()->routeIs($link['route']) || request()->routeIs(explode('.', $link['route'])[0] . '.*') 
                ? 'bg-gradient-to-r from-blue-600 to-cyan-400 text-white shadow-lg shadow-blue-500/30 translate-x-1' 
                : 'text-slate-500 hover:text-white hover:bg-white/5' }} gap-x-3">

            <svg class="h-5 w-5 shrink-0 transition-transform duration-300 {{ request()->routeIs($link['route']) ? 'scale-110' : 'group-hover:scale-110' }}" 
...
<li class="mt-8 mb-4 px-4">
    <span class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Administration</span>
</li>
...
<li>
    <a href="{{ route($link['route']) }}" 
       ...
       {{ request()->routeIs($link['route']) || request()->routeIs($link['route'].'*') 
            ? 'bg-gradient-to-r from-blue-600 to-cyan-400 text-white shadow-lg shadow-blue-500/30 translate-x-1' 
            : 'text-slate-500 hover:text-white hover:bg-white/5' }} justify-between">
        ...
        @if($link['route'] === 'admin.access-requests.index')
            <span x-show="pendingAccessCount > 0" x-text="pendingAccessCount" class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-cyan-500 text-[10px] font-black text-slate-950 ring-2 ring-[#030712]" style="display: none;">
...
<li class="mt-8 mb-4 px-4">
    <span class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Compte</span>
</li>

<li>
    <a href="{{ auth()->user()->isAdmin() ? route('admin.profile.edit') : route('profile.edit') }}" class="group flex items-center rounded-xl py-2 px-3 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 {{ request()->routeIs('profile.edit') || request()->routeIs('admin.profile.edit') ? 'bg-gradient-to-r from-blue-600 to-cyan-400 text-white shadow-lg shadow-blue-500/30 translate-x-1' : 'text-slate-500 hover:text-white hover:bg-white/5' }} gap-x-3">
        <svg class="h-5 w-5 shrink-0 transition-transform duration-300 {{ request()->routeIs('profile.edit') || request()->routeIs('admin.profile.edit') ? 'scale-110' : 'group-hover:scale-110' }}" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
...
@if(auth()->user()->isAdmin())
<li>
    <a href="{{ route('admin.settings') }}" class="group flex items-center rounded-xl py-2 px-3 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 text-slate-500 hover:text-white hover:bg-white/5 gap-x-3">
        <svg class="h-5 w-5 shrink-0 transition-transform duration-300 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
...
<li>
    <form method="POST" action="{{ route('logout') }}" class="m-0">
        @csrf
        <button type="submit" class="w-full group flex items-center rounded-xl py-2 px-3 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 text-rose-500 hover:bg-rose-500/10 gap-x-3">
            <svg class="h-5 w-5 shrink-0 transition-transform duration-300 group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
