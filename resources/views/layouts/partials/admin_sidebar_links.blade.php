...
@foreach($mainLinks as $link)
    ...
    <li>
        <a href="{{ route($link['route']) }}" 
           @click.prevent="if (window.innerWidth < 1024) { $dispatch('close-sidebar'); setTimeout(() => window.location.href = '{{ route($link['route']) }}', 100); } else { window.location.href = '{{ route($link['route']) }}'; }"
