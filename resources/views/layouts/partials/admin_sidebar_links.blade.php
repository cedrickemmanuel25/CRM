...
@foreach($mainLinks as $link)
    ...
    <li>
        <a href="{{ route($link['route']) }}" 
           class="group flex items-center rounded-lg py-1.5 px-2.5 text-xs font-medium leading-6 transition-all duration-200 
           ...
           :class="(sidebarCollapsed && !{{ ($isMobile ?? false) ? 'true' : 'false' }}) ? 'justify-center !px-0 !translate-x-0' : 'gap-x-2.5'">
            ...
            <span x-show="!sidebarCollapsed || {{ ($isMobile ?? false) ? 'true' : 'false' }}" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                {{ $link['label'] }}
            </span>
        </a>
    </li>
...
<li class="mt-6 mb-3 px-3" x-show="!sidebarCollapsed || {{ ($isMobile ?? false) ? 'true' : 'false' }}">
    <span class="text-[10px] font-bold uppercase tracking-widest text-gray-500">Administration</span>
</li>
<li class="mt-6 mb-3 flex justify-center" x-show="sidebarCollapsed && !{{ ($isMobile ?? false) ? 'true' : 'false' }}">
...
<li>
    <a href="{{ route($link['route']) }}" 
       ...
       {{ request()->routeIs($link['route']) || request()->routeIs($link['route'].'*') 
            ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md shadow-indigo-500/20 translate-x-1' 
            : 'text-gray-600 hover:text-gray-900 hover:bg-gray-200' }}"
       :class="(sidebarCollapsed && !{{ ($isMobile ?? false) ? 'true' : 'false' }}) ? 'justify-center !px-0 !translate-x-0' : 'justify-between'">
        <div class="flex items-center" :class="(sidebarCollapsed && !{{ ($isMobile ?? false) ? 'true' : 'false' }}) ? 'justify-center' : 'gap-x-2.5'">
            <svg class="h-5 w-5 shrink-0 transition-transform duration-200 {{ request()->routeIs($link['route']) ? 'scale-110' : 'group-hover:scale-110' }}" 
                 fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $link['icon'] }}" />
            </svg>
        ...
        @if($link['route'] === 'admin.access-requests.index')
            <span x-show="pendingAccessCount > 0 && (!sidebarCollapsed || {{ ($isMobile ?? false) ? 'true' : 'false' }})" x-text="pendingAccessCount" class="inline-flex items-center justify-center h-5 w-5 rounded-full bg-rose-500 text-[10px] font-bold text-white ring-2 ring-white" style="display: none;">
                {{ isset($pendingAccessCount) ? $pendingAccessCount : '' }}
            </span>
            <!-- Red dot for collapsed state -->
            <span x-show="pendingAccessCount > 0 && sidebarCollapsed && !{{ ($isMobile ?? false) ? 'true' : 'false' }}" class="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white" style="display: none;"></span>
        @endif
